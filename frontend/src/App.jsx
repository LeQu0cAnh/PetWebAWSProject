...
// Chỉ block khi isLoading=true VÀ đã authenticated (cần verify banned status).
// User chưa đăng nhập (isAuthenticated=false) không cần đợi → render app ngay.
function BannedGuard({ children }) {
  const { isAuthenticated, isLoading, dbUser } = useAuth();
  // Chỉ show loader khi đang xác thực một session đã có
  // (tránh block app với user chưa đăng nhập)
